...
export default function NewListingBid() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await fetch(`${process.env.REACT_APP_AUCTIONS_ENDPOINT}add/${email}`, {
                method: "POST",
                credentials: "include",
                headers: {
    ...
    const renderMakeSuggestion = (suggestion: string) => (
        <div onClick={() => {
            formik.setFieldValue("make", suggestion);
            formik.setFieldValue("model", "");
            setTempMake(suggestion)
        }}>
            {suggestion}
        </div>
    );
    ...
    const renderModelSuggestion = (suggestion: string) => (
        <div onClick={() => {
            formik.setFieldValue("model", suggestion);
            setTempModel(suggestion);
            setRenderModelSuggestions(false)
        }}>
            {suggestion}
        </div>
    );
        ...
        <div className="new-listing-page p-4">
            ...
            <form className="new-listing-form" onSubmit={formik.handleSubmit}>
                ...
                <div className="relative">
                    <Autosuggest
                        ...
                        inputProps={{
                            ...
                            onFocus: () => {
                                setRenderMakeSuggestions(true);
                                setRenderModelSuggestions(false)
                            },
                            onBlur: () => {
                                setRenderMakeSuggestions(false);
                                setRenderModelSuggestions(false);
                                setTempModel("")
                            },
                        }}
                    ...
                    />
                    {formik.values.make && (
                        <button onClick={handleResetMake}
                ...
                <div className="relative">
                    <Autosuggest
                        ...
                        inputProps={{
                            ...
                            onFocus: () => {
                                setRenderModelSuggestions(true);
                                setRenderMakeSuggestions(false)
                            },
                            onBlur: () => {
                                setRenderModelSuggestions(false);
                ...
                {formik.errors.engine_capacity ?
                    <div className="text-red-500">{formik.errors.engine_capacity}</div> : null}
                <label htmlFor="fuel" className="block mb-1 font-bold">Fuel</label>
                <select id="fuel" name="fuel" className="w-full border p-2 mb-2" onChange={formik.handleChange}
                ...
                <select id="drive" name="drive" className="w-full border p-2 mb-2" onChange={formik.handleChange}
                ...
                {formik.errors.registration_number ?
                    <div className="text-red-500">{formik.errors.registration_number}</div> : null}
                ...
                />
                <label htmlFor="start" className="block mb-1 font-bold">Start Time</label>
                <input
                    id="start"
                    name="start"
                    type="datetime-local"
                ...
                {formik.errors.telephone_number ?
                    <div className="text-red-500">{formik.errors.telephone_number}</div> : null}
                <LocationInput
                    onLocationChange={handleLocationChange}
                />
                <button type="submit"
                        className="border rounded p-2 bg-teal-500 text-black hover:bg-teal-600 transition duration-300 w-full font-bold">Submit
                </button>
            </form>
        </div>
    );
