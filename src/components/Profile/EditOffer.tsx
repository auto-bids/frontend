    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (props.type === "auction") {
                delete (valuesWithPhotos as Partial<any>).price;
            }

            const uri = props.type === "offer" ? `${process.env.REACT_APP_API_BASE_URL}/${props.category}/edit/me` : `${process.env.REACT_APP_AUCTIONS_ENDPOINT}edit/${props.email}/${props.id}`;
                ...
                <div className="mb-4">
                    ...
                    {formik.values.photos?.map((photo, index) => (
                        <div key={index} className="flex items-center mb-2">
                            {photo !== "" && (
                                <img src={photo} alt={`${index + 1}`} className="w-16 h-16 object-cover rounded"/>
                            )}
                            {photo === "" && (
